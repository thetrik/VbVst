        ...
        With m_tPattern.tKeys(lIndex)
            ...
            If .lValue >= lStartRow And .lValue <= lEndRow Then
                ...
                If bDraw Then
                    ...
                    lVelPos = (tArea.lWidth - 2) * .dVelocity + tArea.lLeft + 1
                    
                    SetDCPenColor hdc, &HF0F0FF
                    
                    MoveToEx hdc, lVelPos, tArea.lTop + 1, ByVal NULL_PTR
                    LineTo hdc, lVelPos, tArea.lTop + tArea.lHeight - 1
                    ' // TextOut hdc, tArea.lLeft, tArea.lTop, CStr(lIndex), Len(CStr(lIndex))
                    SetDCPenColor hdc, &H303030
...
Private Sub RedrawActiveKey()
    ...
    If hdc Then
        ...
        If m_eMouseMode = MTM_VELOCITY Then
            
            SaveDC hdc
            SelectObject hdc, GetStockObject(DC_PEN)
            SetDCPenColor hdc, &HF0F0FF
            
            lPos = m_tActiveKey.dVelocity * (tArea.lWidth - 2) + tArea.lLeft + 1
            MoveToEx hdc, lPos, tArea.lTop + 1, ByVal NULL_PTR
            LineTo hdc, lPos, tArea.lTop + tArea.lHeight - 1
        ...
        If Not m_bLocked Then
            ...
            If PtIsInArea(tPt.x, tPt.y, m_tClientArea) Then
                ...
                Else
                    ...
                    m_tActiveKey.dVelocity = m_tPattern.tKeys(lKey).dVelocity
                    
                    m_tMouseOffest.y = tPt.y
                        
                    If iShift And vbCtrlMask Then
                        
                        m_eMouseMode = MTM_VELOCITY
...
Private Sub UserControl_MouseMove( _
    ...
    If iButton = vbLeftButton Then
        ...
        ElseIf m_eMouseMode = MTM_VELOCITY Then
        
            lDelta = m_tMouseOffest.y - tPt.y
            
            If lDelta = 0 Then
                Exit Sub
            End If
            
            m_tActiveKey.dVelocity = m_tPattern.tKeys(m_tActiveKey.lSelKey).dVelocity + lDelta / 300
            
            If m_tActiveKey.dVelocity > 1 Then
                m_tActiveKey.dVelocity = 1
            ElseIf m_tActiveKey.dVelocity < 0 Then
                m_tActiveKey.dVelocity = 0
            End If
...
Private Sub UserControl_MouseUp( _
    ...
    If iButton = vbLeftButton Then
        ...
        Case MTM_VELOCITY
            ...
            m_tPattern.tKeys(m_tActiveKey.lSelKey).dVelocity = m_tActiveKey.dVelocity
            DrawKey m_tActiveKey.lSelKey, False
            InvalidateRow m_tActiveKey.lValue
...
Private Sub PutActiveKeyToPattern()
    ...
    Dim tNewKeys()  As tKey
    Dim dActEnd     As Double
    Dim tNewKey     As tKey
    Dim bAddNewKey  As Boolean
    Dim bPasted     As Boolean
    Dim bNewPasted  As Boolean
    Dim bSkipRegion As Boolean
    ...
    With m_tPattern
        ...
        For lIndex = 0 To .lNumOfKeys - 1
            ...
            With .tKeys(lIndex)
                ...
                If .dPos > m_tActiveKey.dPos And Not bPasted Then
                    
                    With tNewKeys(lDstIndex)
                        .dPos = m_tActiveKey.dPos
                        .dLength = m_tActiveKey.dLength
                        .lValue = m_tActiveKey.lValue
                        .dVelocity = m_tActiveKey.dVelocity
                    End With
                ...
                If bAddNewKey And Not bNewPasted Then
                    
                    If .dPos > tNewKey.dPos Then
                    
                        tNewKeys(lDstIndex) = tNewKey
                        lDstIndex = lDstIndex + 1
                        bNewPasted = True
                ...
                If .lValue = m_tActiveKey.lValue And Not bSkipRegion Then
                    
                    If .dPos + .dLength <= m_tActiveKey.dPos Then
                        tNewKeys(lDstIndex) = m_tPattern.tKeys(lIndex)
                        lDstIndex = lDstIndex + 1
                    ElseIf .dPos < m_tActiveKey.dPos And .dPos + .dLength > m_tActiveKey.dPos Then
                        ...
                        If .dPos + .dLength > dActEnd Then
                            ...
                            tNewKey.dLength = .dPos + .dLength - dActEnd
                            
                            If tNewKey.dLength > KEY_MIN_LENGTH Then
                            
                                bAddNewKey = True
                                tNewKey.dPos = dActEnd
                                tNewKey.lValue = .lValue
                                tNewKey.dVelocity = .dVelocity
                    ...
                    ElseIf .dPos <= dActEnd And .dPos + .dLength > dActEnd Then
                        ...
                        If .dLength > KEY_MIN_LENGTH Then
                            tNewKey = m_tPattern.tKeys(lIndex)
                            bAddNewKey = True
                        End If
                        
                    ElseIf .dPos > dActEnd Then
